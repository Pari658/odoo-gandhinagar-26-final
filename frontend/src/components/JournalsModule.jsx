...
export default function JournalsModule() {
  ...
  const handleSaveJournal = async (formData) => {
    ...
    try {
      ...
      if (editData) {
        const updated = await apiRequest('PUT', `/journals/${editData.id}`, payload);
        setMessage({ type: 'success', text: `Journal '${updated.name}' updated successfully!` });
      } else {
        const created = await apiRequest('POST', '/journals', payload);
        setMessage({ type: 'success', text: `Journal '${created.name}' created successfully!` });
      }
      ...
      {view === 'detail' ? (
        <>
          <JournalDetail 
            ...
            onDelete={isPrivileged ? (id) => setDeleteTarget(journals.find(j => j.id === selectedId)) : null}
          />
          {/* Delete Confirmation Modal reused for detail view */}
          {deleteTarget && (
